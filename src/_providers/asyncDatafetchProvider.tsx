...
import { triggerBusinessDataLongFetch } from '@graphql/mutations';
  ...
  const triggerFetch = async (businessId: string, refreshAferFetch = true) => {
    try {
      ...
      if (selectedBusiness && !refreshAferFetch) {
        updateBusinessAdAccount(selectedBusiness?.business_id as string, {
          ...selectedBusiness,
      ...
      const handler = subscription.subscribe({
        next: async () => {
          ...
          if (selectedBusiness && refreshAferFetch) {
            const updated = businessRef.current as Business;
            updateBusinessAdAccount(selectedBusiness?.business_id as string, {
              ...updated,
              last_data_refreshed: dayjs().format(),
            });
      ...
      await API.graphql({
        ...graphqlOperation(triggerBusinessDataLongFetch, {
