...
import { getAccountsCampaigns } from '@graphql/queries';
...
import { ConnectCampaignsModal } from '@components/modals/tailwindTypes';
...
import ManualTab from '@components/settings/tracking/tab';
...
const TrackingSetup: FC<Props> = ({
  ...
  const { shopifyConnected, businessProfile, selectedBusiness } =
    useBoundStore();
  ...
  const getCampaigns = async () => {
    ...
    try {
      const response: any = await API.graphql(
        graphqlOperation(getAccountsCampaigns, {
          getAccountsCampaignsInput: {
            business_id: businessProfile?.profile?.business_id,
            itemType: 'ads',
      ...
      const data = response.data?.getAccountsCampaigns?.data;
  ...
  const updateSelectedAdsAccounts = useCallback(async () => {
                ...
                API.graphql(
                  graphqlOperation(setUtmValuesSocialCampaign, {
                    setUtmValuesSocialCampaignInput: {
                      adId: item.id,
                      businessId: businessProfile?.profile?.business_id,
                      itemType: 'ads',
                      source: 'facebook',
                    },
                  }),
    ...
    selectedGoogleAds,
    selectedTiktokAds,
    businessProfile?.profile?.business_id,
    setIsConnecting,
  ]);
            ...
            <>
              <div className="mt-5 mx-2">
                {businessProfile?.profile?.google_ad_account_id && (
                  <ConnectedAcounts
                    type="Google"
                    acountName={businessProfile.profile?.google_ad_account_id}
    ...
    <>
      ...
      <TailwindModal id="connectCampaignsSelectively">
        <ConnectCampaignsModal
          ...
          tiktokAds={
            businessProfile?.profile?.tik_tok_ad_account_name ? tiktokAds : []
          }
          googleAds={
            businessProfile?.profile?.google_ad_account_id ? googleAds : []
          }
