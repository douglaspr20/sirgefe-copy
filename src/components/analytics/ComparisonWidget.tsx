...
import { ValidSourceAnalytics } from '@interfaces/source';
...
import CompareModal from '@components/modals/tailwindTypes/CompareModal';
import { MarketingSources } from '@sirge-io/sirge-types';
import { API, graphqlOperation } from 'aws-amplify';
import { getBusinessAdcomparisonData } from '@graphql/queries';
...
const ComparisonWidget: React.FC<Props> = ({ source, isSending }) => {
  ...
  const fetchComparisonProduct = async (businessId: string) => {
    ...
    const response: any = await API.graphql(
      graphqlOperation(getBusinessAdcomparisonData, {
    ...
    const adcomparisonData =
      response?.data?.getBusinessAdcomparisonData?.data || [];
  ...
  useEffect(() => {
    if (selectedBusiness?.business_id) {
      fetchComparisonProduct(selectedBusiness.business_id!);
    }
  }, [source, selectedBusiness?.business_id, comparison, adComparisonPeriod]);
