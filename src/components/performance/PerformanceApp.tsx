...
const PerformanceCampaignsPage = () => {
  ...
  useEffect(() => {
    const fetchMemberCount = async () => {
      try {
        const response: any = await API.graphql(
          graphqlOperation(getMemberCountByAdGroupIDsNew, {
        ...
        if (!response.data.getMemberCountByAdGroupIDsNew.data?.error) {
          setMemberCount(response.data.getMemberCountByAdGroupIDsNew.data);
        }
      } catch (error) {
        console.log(error);
      }
