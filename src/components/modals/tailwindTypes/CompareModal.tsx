...
const CompareModal: FC<Compare> = ({
  ...
  const fetchAds = async (page?: number) => {
    ...
    const comparisionData = response.data.getBusinessAdcomparisonData;

    setData(comparisionData?.data || []);
                    ...
                    <tbody>
                      {data?.map((item: Item, index: number) => (
                        ...
                        >
                          ...
                          <td className="text-textSecondaryColor px-2 py-3 border-b border-r border-extraLightColor last:border-r-0">
                            {' '}
                            {item.total_amount_spent?.toLocaleString('en-US', {
                              style: 'currency',
                              currency: 'USD',
                          ...
                          <td className="text-textSecondaryColor px-2 py-3 border-b border-r border-extraLightColor last:border-r-0">
                            {' '}
                            {item.roas
                              ? `${parseFloat(item.roas?.toString())?.toFixed(
                                  2,
                                )}X`
                              : '0.00X'}{' '}
                          </td>
