'use client';
import { getTrackableCopyOrders } from '@graphql/queries';
  ...
  const fetchAllOrders = async () => {
    try {
      ...
      const response: any = await API.graphql(
        graphqlOperation(getTrackableCopyOrders, {
          getTrackableCopyOrdersInput: {
      ...
      if (response?.data?.getTrackableCopyOrders?.error) {
        throw new Error(response.data.getTrackableCopyOrders.error.message);
      }

      setOrders(response?.data?.getTrackableCopyOrders?.data || []);
      setPageTotal(response?.data?.getTrackableCopyOrders?.numberPages || 1);
    } catch (error) {
